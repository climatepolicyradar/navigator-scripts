...
def connect_to_postgres(database_url: str) -> Union[sqlalchemy.engine.Connection, bool]:
    try:
        ...
        _LOGGER.exception("Failed to connect to Postgres")
        _LOGGER.error(
            "DATABASE_URL parameter with value '%s' incorrect", SQLALCHEMY_DATABASE_URI
        )
        return False


def get_whole_database_dump(
    conn: sqlalchemy.engine.Connection, sql_query_file: str = "whole_data_dump.sql"
):
